...
def cmd_demo(args: argparse.Namespace) -> int:
    session = demo.build_demo_session(with_descriptors=not args.no_descriptors)
...
def cmd_scan_stems(args: argparse.Namespace) -> int:
    if not os.path.isdir(args.folder):
        ...
        return 2
    session = _build_from_folder(args.folder, with_descriptors=not args.no_descriptors)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_demo = sub.add_parser("demo", help="Build the built-in synthetic demo session.")
    p_demo.add_argument("--out", help="Directory to write JSON exports into.")
    p_demo.set_defaults(func=cmd_demo)

    p_scan = sub.add_parser("scan-stems", help="Scan a folder of exported stems.")
    ...
    p_bundle = sub.add_parser("export-bundle", help="Scan a folder and write the full bundle JSON.")
